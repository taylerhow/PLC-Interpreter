...
(define (test-subst-leftmost)
	(let
		([correct '((((a b (c () (d new (f g)) h)) i)))]
		[answers 
	 		(list
	 			(eval-one-exp '
	  				(letrec 
	  					([apply-continuation  (lambda (k . list-of-values) (apply k list-of-values))]
		   				[subst-left-cps
	      					...
	      					(subst-left-cps
			      				...
			      				(lambda (changed-s)
									(subst-left-cps
										...
										(lambda () (list changed-s))			; This is the one that BREAKS EVERYTHING
									)
								)

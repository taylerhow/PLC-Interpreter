; Tayler How, Jesse Shellabarger, & Chase Bishop, Assignment 14 10/21/15
	...
	(lambda (e env)
		(cases expression e
			...
			[improper-lambda-exp (defined-args undefined-args bodies)				; Simply makes a closure that has the undefined-args as the last variable
				(improper-closure (append (list undefined-args) defined-args) bodies env)
			]
			[while-exp (condition bodies)
				(let loop ([return '()])
						(if (eval-exp condition env)
...
(define improper-closure-vals-init
	(lambda (vars args)
		(let ([num-vars (- (length vars) 1)])
			(let loop ([output '()] [rest args])
				(cond
					[(equal? (length output) num-vars) (append (list rest) output)]
